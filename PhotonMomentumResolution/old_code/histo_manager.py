...
import math
import ROOT
from ROOT import TH1D, TH2D, TH3D, TMath, TH1F, TProfile, TMath
...
def get_significance(h1sig,h1bkg):
    ...
    for i in range(0,n):
        ...
        sig = s/TMath.sqrt(s + 2.*b);
        sig_err = TMath.sqrt( pow(pow(s+2*b,-1/2.) - s/2.*pow(s+2.*b, -3/2) ,2) *pow(s_err,2) + pow( 2*s * pow(s+2*b,-3/2.) ,2) * pow(b_err,2)  );
